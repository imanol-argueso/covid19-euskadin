...
window.onload = function () {
    ...
    getJSON(DATAFILES.BYMUNICIPALITY, function (err, dataJson) {
        if (err != null) {
            ...
            dataJson.byMunicipalityByDate.populationByMunicipalityByDate.forEach(element => {
                let last14municipalityActive = last14municipality.filter(x => x.municipalityCode === '' + element.dimension.countyId + element.dimension.oid + '');
                last14municipalityPositives.push({ municipalityName: last14municipalityActive[0].municipalityName, municipalityCode: last14municipalityActive[0].municipalityCode, positives: last14municipalityActive[0].positives, population: element.values[0], rate: (last14municipalityActive[0].positives * (100000 / element.values[0])).toFixed(0) })
            });
